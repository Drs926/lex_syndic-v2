...
| `LEX-034` | API FastAPI locale mono-utilisateur : `src/lex_syndic/api/fastapi_app.py`. `POST /v1/analyze`, `GET /v1/results/{record_id}`, `GET /health`. Routes `/docs` `/redoc` `/openapi.json` désactivées. Guard texte (vide → 422, > 50 000 chars → 422). `fastapi` + `uvicorn[standard]` ajoutés. 7 fonctions de test / 10 cas FastAPI dans `tests/test_api_fastapi.py`. | 186 tests globaux verts. Aucune persistance, aucune auth, aucun DB. DEC-LEX-034 ajoutée. | **TERMINÉ** — PR #54 |
| `LEX-040` | FastAPI local docs index : `docs/usage/LEX_040_FASTAPI_LOCAL_DOCS_INDEX.md`. | Index documentaire local FastAPI. | **TERMINÉ** — PR #60 / merge `2b43145` |
| `LEX-041` | Réconciliation documentaire de `STATUS.md` après LEX-040. | Gouvernance limitée à `STATUS.md`; aucun code produit, test ou dépendance modifié. | **TERMINÉ** — PR #61 / merge `0452ad2` |
